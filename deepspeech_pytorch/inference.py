...
def transcribe(cfg: TranscribeConfig, save=False):
    ...
    spect_parser = SpectrogramParser(audio_conf=model.audio_conf,
                                     normalize=True)
    csv = pd.read_csv('/home/harveen.chadha/exp_5000/tarini_manifest.csv', header=None)
    csv = csv.iloc[:,0].values

    for file in tqdm(csv):


        decoded_output, decoded_offsets = run_transcribe(audio_path=file,
        ...
        path_to_save = file[:-4] + '_transcription.txt'
        with open(path_to_save, mode='w+', encoding='utf-8') as f:
            f.write(results['output'][0]['transcription'])
