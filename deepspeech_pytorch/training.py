...
def notification(message):
    ...
    requests.post('https://maker.ifttt.com/trigger/epoch_ended/with/key/d400J3Ggc1ba2bA2XJRWmn', data=report)
...
def train(cfg):
    ...
    if cfg.checkpointing.continue_from:  # Starting from previous model
        ...
        with open(to_absolute_path(cfg.data.labels_path), encoding='utf8') as label_file:
    ...
    if OmegaConf.get_type(cfg.optim) is SGDConfig:
        print("SGD Config is selected here")
        ...
        print("input lr was", cfg.optim.learning_rate, " optimizer lr was ", optimizer.param_groups[0]['lr'])
    ...
    print('2.1 LR now is', optimizer.param_groups[0]['lr'])
    ...
    for epoch in range(state.epoch, cfg.training.epochs):
        ...
        for i, (data) in enumerate(train_loader, start=state.training_step):
            ...
            if valid_loss:
                optimizer.zero_grad()
                #print("3 LR is ", optimizer.param_groups[0]['lr'])
                # compute gradient
                ...
                print("Predicted", float_out)
                print("Predicted shape", float_out.shape)
                print("Predicted sizes", output_sizes)
                print("Ground Truth", targets)
                print("Ground truth shape", targets.shape)
                print("Groud truth sizes", target_sizes)
                print("Loss", loss)
                print("Loss value", loss_value)
                message = "lode lag gaye"
        ...
        message = 'Epoch = {}, Loss = {}, WER = {}, CER = {}'.format(epoch+1, state.avg_loss, wer, cer)
        notification(message)
